...
<!--         CLICK NUM & POST TIME            -->			
			<small class="gray">
				<a href="<?=WEB_SITE;?>/member/<?=$row_post['username'];?>"><?=$row_post['username'];?></a> · <?php  echo date('Y-m-d,H:i:s' , $row_post['ctime'])?> 发布 · <?=$row_post['ccount'];?> 次点击 &nbsp; 
			</small>
		</div>
		<div class="sep20 "></div>    
    ...
    <div class="cell fl clearFix">
        ...
        <div class="topic_content"><?php  echo $Parsedown->text($row_post['content']);?></div>
...
<!--主题最后回复时间-->
		...
		$last_time = $last[0]['ctime']?>

        
<!--         RECEIVE COUNT            -->
		<span class="gray"><?=$rcount[0]['count(pid)'];?> 回复 &nbsp;
			<strong class="snow">|</strong> &nbsp;直到 <?php  echo date('Y-m-d H:i:s' , $last_time)?> +08:00
		</span>
    </div>
    ...
    <!--         回复列表            -->
        ...
        <div id="" class="cell fl clearFix">   		
			...
			<div class="reply_text fr clearFix ">
				...
				<span class="fl fade small"><?php  echo date('Y-m-d,H:i:s' ,$value['ctime'])?></span> 
				<div class="sep5"></div>
				<div class="reply_content fl"><?php  echo $Parsedown->text($value['content']);?></div>				
			</div>		
        </div>
    	<?php endforeach;?>
    <?php else: ?>
...
<!--         RECEIVE TEXTAREA            -->
		...
		<?php  echo '尚未登录，不能发表回复，请<a href="<?=WEB_SITE;?>/html/signin.html">登录</a>'?>
			</div>
